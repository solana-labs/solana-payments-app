...
interface Props<R extends { [key: string]: any }, CO extends (keyof R)[]> {
  ...
  className?: string;
  /**
   * The data to be displayed. Each item in the array represents a row. Each
   * row's data is in the form of an object with keys mapping to values
   */
  data: R[];
  /**
  ...
  /**
   * The number of rows to be displayed in each page.
   */
  pageSize: number;
  /**
...
>(props: Props<R, CO>) {
  ...
  const numPages = Math.ceil(props.data.length / props.pageSize);
  const rowsInPage = props.data.slice(
    page * props.pageSize,
    (1 + page) * props.pageSize
  );
    ...
    <div className={twMerge(props.className)}>
      <div className="grid grid-cols-[minmax(max-content,1fr),minmax(0,max-content)] overflow-hidden">
        <div className="border-r border-gray-200 shadow-lg md:border-none md:shadow-none">
          <div className={twMerge(STYLES.HEADER, "pr-6")}>
            {props.headers[props.columns[0]]}
          </div>
        ...
        <div className="overflow-x-auto">
          ...
          >
            {props.columns.slice(1).map((column, i) => (
              <div
                className={twMerge(STYLES.HEADER, i > 0 ? "pl-12" : "pl-6")}
                key={i}
              >
      ...
      <div className="flex items-center justify-between mt-4 md:mt-7">
        ...
        <PaginatedTableArrowButton
          ...
          onClick={() =>
            setPage((cur) => (cur < numPages - 1 ? cur + 1 : numPages - 1))
          }
        />
      </div>
    </div>
