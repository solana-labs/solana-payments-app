...
export type PaymentStatusResponse = InferType<
    typeof paymentStatusResponseScheme
>

export const parseAndValidatePaymentStatusRequest = (
    paymentInitiationRequestBody: any
): PaymentStatusRequest => {
    let parsedPaymentRequest: PaymentStatusRequest
    try {
        parsedPaymentRequest = paymentStatusRequestScheme.cast(
            paymentInitiationRequestBody
        ) as PaymentStatusRequest
    } catch (error) {
